...
**Known, confirmed gaps as of the 2026-08-13 Colors-page audit, not yet resolved:**
- **The `8%-opacity black` card border/divider convention (§2.2) has no exact-match token.** `Semantic/Border/Divider` exists but is `12%` opacity, a different value used for a different real component (the `<Divider>` line itself). Until a `Documentation/Border/*`-style token is minted at the correct `8%` value, this literal is an accepted exception — do not force-bind it to `Semantic/Border/Divider`, which would silently shift the value.
