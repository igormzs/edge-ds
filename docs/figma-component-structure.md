...
### 3.1 Fill color must be tokenized too, not just "set explicitly"

**Ratified 2026-08-13, supersedes the "set explicitly" language above.** Text fill color is no longer exempt: every text fill in Foundations-page documentation (Colors, Typography, and any future Foundations page — the rule almost certainly should extend to component pages too, but that hasn't been retrofitted everywhere yet) must be bound to a real color variable (e.g. `Semantic/Text/Primary`, `Documentation/Text/Muted`), never a literal hex, even one that happens to match exactly. `textStyleId` still only governs font family/weight/size/spacing/line-height/case (unchanged) — color is a separate, now-mandatory binding on top of it.

### 3.2 The same rule extends beyond text — backgrounds, borders, spacing

Any styling value used to build documentation chrome — background fills, border/stroke colors, spacing where a real spacing token exists — must point at a real, existing token, never a hardcoded literal that happens to look right. **If a value is needed and no matching token exists yet, flag it explicitly. Do not invent a new token unilaterally, and do not force-bind to a token whose value doesn't actually match.** This is stricter than §2.2's own literal `#FFFFFF`/`8% black` description above: where a real token already exists for the same value (`Semantic/Surface/Paper` for card backgrounds, `Brand/White`, `Neutral/Grey/900`, `Neutral/Black`, etc.), always bind to it instead of the literal description in this doc. Where no token exists yet for a specific literal, that literal remains a disclosed, accepted exception until a token is minted — do not treat "no token exists" as license to hardcode silently; the absence itself must be flagged.

**Known, confirmed gaps as of the 2026-08-13 Colors-page audit:**
- **Resolved, 2026-08-13, during Select's own documentation-chrome token-binding pass:** the `8%-opacity black` card border/divider convention (§2.2) was believed to have no exact-match token as of the note above — that turned out to be incomplete, not permanent. `Documentation/Border/Subtle` already exists (resolves to `grey/300`, `#E0E0E0`) and is already the real, live binding Link's own shipped Documentation frame uses for this exact role — checked directly against Link's Divider and Card-stroke bindings before deciding, not assumed. Bound Select's 24 instances (8 section-title dividers, 8 card/table container strokes, 8 Key Props row strokes) to it, accepting the resulting ~4% lightness delta from the original literal (`rgba(0,0,0,0.08)` on white ≈ `#EBEBEB` vs. the token's `#E0E0E0`) deliberately rather than minting a new token or leaving it unbound — small enough on a white card background not to warrant either. Any other page still carrying this literal unbound should follow the same precedent (bind to `Documentation/Border/Subtle`), not re-litigate the gap as still-open.
- **No official text style exists for granular data-label roles** (a color swatch's weight number or hex-value caption, for instance) — the existing role table above (Page Title, Section Headers, Body, Caption, etc.) doesn't cover this granularity. Same treatment as the existing monospace-token exception two paragraphs up: leave as literal/manual styling, don't force an ill-fitting role onto it, flag it as a gap if the pattern recurs enough to justify minting a dedicated token.
- **Spacing has no established binding practice anywhere in this file yet.** A real `Sizing` variable collection exists (`Sizing/1`–`Sizing/12`, an 8px-based scale, confirmed real and already referenced once in this doc's own Code Connect example in §5.6) but a file-wide scan found it is not bound to a single padding/gap property anywhere across either Colors frame — despite exact-value matches existing for the majority of common spacing values (8/16/24/32/40/48/64px). This is a pre-existing, project-wide gap, not specific to Colors — fixing it properly means a deliberate, broader retrofit pass across every already-built page, not a one-off fix on a single Foundations page that would leave every other page inconsistent. Flagged, not fixed.
